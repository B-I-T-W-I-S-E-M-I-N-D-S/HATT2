...
def train(opt): 
    ...
    for n_epoch in range(opt['epoch']):   
        ...
        # Save checkpoint, handling DataParallel's module state
        state = {
            'epoch': n_epoch + 1,
            'state_dict': model.module.state_dict() if isinstance(model, nn.DataParallel) else model.state_dict()
        }
        torch.save(state, os.path.join(opt["checkpoint_path"], f"{opt['exp']}_checkpoint_{n_epoch+1}.pth.tar"))
        if IoUmAP_5 > getattr(model.module if isinstance(model, nn.DataParallel) else model, 'best_map', 0):
            setattr(model.module if isinstance(model, nn.DataParallel) else model, 'best_map', IoUmAP_5)
            torch.save(state, os.path.join(opt["checkpoint_path"], f"{opt['exp']}_ckp_best.pth.tar"))
            
    writer.close()
    return getattr(model.module if isinstance(model, nn.DataParallel) else model, 'best_map', 0)
